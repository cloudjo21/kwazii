...
class BaseFdeEncoder(torch.nn.Module, abc.ABC):
    ...
    def __init__(self, encoder_model_path: str, **kwargs):
        ...
        self.fde_configs = {
            PromptType.QUERY: config.FdeConfig.apply_with_prompt_type(PromptType.QUERY),
            PromptType.PASSAGE: config.FdeConfig.apply_with_prompt_type(PromptType.PASSAGE)
        }
        ...
        if input_type is None:
            if isinstance(inputs, str):
                ...
                if isinstance(first_item, str):
                    ...
                    raise ValueError("Mixed input types not supported in unified encode method")
            else:
