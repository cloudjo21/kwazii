...
from asmr.index.fields import (
    SparseTextFieldIndex, 
    DenseTextFieldIndex, 
    DenseImageFieldIndex,
    DocumentIndex
)
...
class MockEncoder:
    ...
    def encode_image(self, images):
        # Return mock embeddings
        embeddings = np.random.rand(len(images), self.dimension).astype(np.float32)
        return embeddings
...
class TestFieldIndexing(unittest.TestCase):
    ...
    def _define_field_configs(self) -> Dict[str, FieldConfig]:
        ...
        return {
            ...
            "title_dense": FieldConfig(
                ...
                faiss_index_path=os.path.join(self.test_dir, "title_dense.faiss")
            ),
            ...
            "content_dense": FieldConfig(
                ...
                representation_type=RepresentationType.DENSE,
                model_path="bert-base-uncased",
                faiss_index_path=os.path.join(self.test_dir, "content_dense.faiss")
            ),
            ...
            "review_text_dense": FieldConfig(
                ...
                representation_type=RepresentationType.DENSE,
                model_path="bert-base-uncased",
                faiss_index_path=os.path.join(self.test_dir, "review_text_dense.faiss")
            ),
            ...
            "review_image": FieldConfig(
                ...
                faiss_index_path=os.path.join(self.test_dir, "review_image.faiss")
            )
        }
    ...
    def _initialize_field_indices(self):
        ...
        for field_name in ["title_sparse", "content_sparse", "review_text_sparse"]:
            config = self.field_configs[field_name]
            bm25_index = MockBM25Index()
            self.field_indices[field_name] = SparseTextFieldIndex(config, bm25_index)
        
        # Initialize dense text indices
        for field_name in ["title_dense", "content_dense", "review_text_dense"]:
            config = self.field_configs[field_name]
            self.field_indices[field_name] = DenseTextFieldIndex(config, self.encoder)
        
        # Initialize dense image index
        config = self.field_configs["review_image"]
        self.field_indices["review_image"] = DenseImageFieldIndex(config, self.encoder)
    ...
    def test_field_config_creation(self):
        ...
        # Test sparse configs
        sparse_configs = ["title_sparse", "content_sparse", "review_text_sparse"]
        for config_name in sparse_configs:
            config = self.field_configs[config_name]
            self.assertEqual(config.representation_type, RepresentationType.SPARSE)
            self.assertEqual(config.tokenizer_type, TokenizerType.SPLIT)
        
        # Test dense configs
        dense_configs = ["title_dense", "content_dense", "review_text_dense", "review_image"]
        for config_name in dense_configs:
            config = self.field_configs[config_name]
            self.assertEqual(config.representation_type, RepresentationType.DENSE)
    ...
    def test_sample_documents_structure(self):
        ...
        required_fields = ["doc_id", "title", "content", "review_text", "review_image"]
    ...
    def test_indexing_pipeline(self):
        ...
        self.field_indices["review_text_dense"].add_documents(doc_ids, review_texts)
        
        # Test dense image indexing
        print("Indexing dense image field...")
        # Mock image objects for testing
        mock_images = [Image.open(f"mock_{img}") for img in review_images]
        self.field_indices["review_image"].add_documents(doc_ids, mock_images)
    ...
    def test_save_indices(self):
        ...
        # Save dense indices
        dense_fields = ["title_dense", "content_dense", "review_text_dense", "review_image"]
    ...
    def test_document_index_integration(self):
        ...
        expected_fields = [
            "title_sparse", "title_dense", 
            "content_sparse", "content_dense",
            "review_text_sparse", "review_text_dense",
            "review_image"
        ]
