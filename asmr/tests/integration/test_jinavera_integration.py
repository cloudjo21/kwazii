...
class TestJinaveraIntegration(unittest.TestCase):
    ...
    def _initialize_encoder(cls):
        ...
        if HAS_ACTUAL_DEPS and os.path.exists(cls.encoder_model_path):
            try:
                print(f"Initializing real Jinavera model from {cls.encoder_model_path}")
    ...
    def _create_sample_images(cls):
        ...
        sample_images = [
            ("travel_sunset.jpg", "A beautiful sunset over mountains", (255, 200, 150)),
            ("city_skyline.jpg", "Modern city skyline at night", (50, 100, 200)),
            ("nature_forest.jpg", "Dense green forest landscape", (50, 150, 50)),
            ("beach_scene.jpg", "Tropical beach with palm trees", (200, 220, 255)),
    ...
    def test_text_encoding_single(self):
        ...
        if hasattr(self.encoder, 'encode_text') and HAS_ACTUAL_DEPS:
            ...
            query_embedding = self.encoder.encode_text([test_text], PromptType.QUERY)
            ...
            passage_embedding = self.encoder.encode_text([test_text], PromptType.PASSAGE)
            self.assertEqual(passage_embedding.shape[0], 1)
            self.assertEqual(passage_embedding.shape[1], query_embedding.shape[1])
    ...
    def test_image_encoding_single(self):
        ...
        self.assertTrue(image_path.exists(), f"Sample image not found: {image_path}")
        ...
        if hasattr(self.encoder, 'encode_image'):
            ...
            for i in range(len(images)):
                for j in range(i+1, len(images)):
                    similarity = np.dot(embeddings[i], embeddings[j])
                    print(f"  Similarity between image {i+1} and {j+1}: {similarity:.4f}")
            
            # Check embedding norms
            norms = np.linalg.norm(embeddings, axis=1)
            print(f"✓ Image embedding norms: min={norms.min():.4f}, max={norms.max():.4f}, mean={norms.mean():.4f}")
    ...
    def test_cross_modal_similarity(self):
        ...
        if not (hasattr(self.encoder, 'encode_text') and hasattr(self.encoder, 'encode_image')):
            print("⚠️  Cross-modal testing requires both text and image encoding")
            return
        ...
        for text_desc, image_file in text_image_pairs:
            ...
            if hasattr(self.encoder, 'encode_text') and HAS_ACTUAL_DEPS:
                text_embedding = self.encoder.encode_text([text_desc], PromptType.QUERY)[0]
            else:
    ...
    def test_end_to_end_workflow(self):
        ...
        reviews = [
            {
                "text": "Amazing sunset view from mountain peak with stunning landscape",
                "image": "travel_sunset.jpg"
            },
            {
                "text": "Vibrant city nightlife with impressive modern architecture",
        ...
        for i, review in enumerate(reviews):
            ...
            # Encode text
            if hasattr(self.encoder, 'encode') and HAS_ACTUAL_DEPS:
                text_emb = self.encoder.encode([review['text']], PromptType.PASSAGE)[0]
            else:
        ...
        text_similarities = [np.dot(query_emb, text_emb) for text_emb in all_text_embeddings]
        best_text_idx = np.argmax(text_similarities)
        print(f"✓ Best text match: Review {best_text_idx + 1} (similarity: {text_similarities[best_text_idx]:.4f})")
        
        # Find best matching image (if available)
        if all_image_embeddings:
            image_similarities = [np.dot(query_emb, img_emb) for img_emb in all_image_embeddings]
            best_image_idx = np.argmax(image_similarities)
            print(f"✓ Best image match: Review {best_image_idx + 1} (similarity: {image_similarities[best_image_idx]:.4f})")
