"""System test: STaRK-MAG MFARAll end-to-end benchmark.

Requires:
  - asmr/data/stark_mag/skb/mag/corpus.json
  - asmr/data/stark_mag/qa/mag/stark_qa/stark_qa.csv
  - asmr/data/stark_mag/qa/mag/split/{train,test}.index
...
_DATA_ROOT = Path(__file__).resolve().parents[2] / "data" / "stark_mag"
_FULL = os.getenv("ASMR_BENCHMARK_TESTS", "0") == "1"
_SKIP = not (_DATA_ROOT / "skb" / "mag" / "corpus.json").exists()


@pytest.mark.skipif(_SKIP, reason=f"STaRK-MAG data missing at {_DATA_ROOT}")
@pytest.mark.system
class TestStarkMagMfarAllBenchmark:
    """System test for STaRK-MAG + MFARAll protocol."""

    def test_mfar_all_benchmark_smoke_or_full(self) -> None:
        from asmr.evaluation.stark_mag_benchmark import run_benchmark
