...
class DocumentRetriever:
    ...
    def __init__(self, fields_retriever: retrievers.QueryRouter, field_configs: Optional[Dict] = None):
    ...
    async def smart_retrieve(self, query: Query, k: int):
        ...
        return await aggregate.smart_retrieve_documents(
            query, 
            self.field_configs, 
            self.fields_retriever, 
            k
    ...
    async def multimodal_retrieve(self, query: Query, k: int):
        ...
        if query.has_image():
            image_query = Query.from_image(query.get_image(), query.get_image_data_type())
