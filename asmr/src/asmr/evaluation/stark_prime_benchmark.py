"""STaRK-Prime MFARAll benchmark runner (hybrid multi-field retrieval)."""
...
MFAR_PRIME_TEST = {
    "BM25": {"hit@1": 0.167, "recall@20": 0.410, "mrr": 0.255},
    "Contriever-FT": {"hit@1": 0.325, "recall@20": 0.600, "mrr": 0.427},
    "MFARAll": {"hit@1": 0.409, "recall@20": 0.683, "mrr": 0.512},
}
...
def train_mfar_adapter(
    train_queries: list[PrimeQuery],
    indexes: PrimeFieldIndexes,
    encoder: HfQueryEncoder,
    *,
    shortlist_k: int,
    epochs: int,
    ...
    logger.info("Training MFARFieldAdapter on %d shortlists", len(ds))
...
def run_benchmark(
    ...
    *,
    ...
    logger.info(
        "Corpus size: %d docs, %d fields", len(corpus.doc_ids), len(PRIME_FIELD_NAMES)
    )

    encoder = HfQueryEncoder(model_name=encoder_name, device=str(device))
    indexes = PrimeFieldIndexes(corpus, encoder)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="STaRK-Prime MFARAll benchmark")
    parser.add_argument(
        "--data-root",
        type=Path,
        default=Path("asmr/data/stark_prime"),
    )
