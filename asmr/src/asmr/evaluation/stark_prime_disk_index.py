"""Disk-backed STaRK-Prime field indexes for memory-efficient MFARAll."""
...
def migrate_index_v21(index_dir: Path) -> None:
    """Add per-field doc_id_mapping.marisa and dense.faiss to a v2 index."""
    manifest_path = index_dir / _MANIFEST
    if not manifest_path.exists():
    ...
    doc_ids = _read_doc_ids(index_dir / _DOC_IDS)
    fields = list(manifest.get("fields", PRIME_FIELD_NAMES))
    ...
    for fi, fname in enumerate(fields):
        ...
        if not mapping_path.exists():
            _save_doc_id_mapping(_build_doc_id_mapping(doc_ids), mapping_path)

        faiss_path = field_dir / _DENSE_FAISS
        memmap_path = field_dir / _DENSE_FILE
        if not faiss_path.exists() and memmap_path.exists():
            logger.info(
                "Migrating field %d/%d %s to FAISS",
                fi + 1,
                len(fields),
                fname,
            )
    ...
    manifest["version"] = 3
    manifest["dense_backend"] = _DENSE_BACKEND_FAISS
    manifest["sparse_backend"] = "csc_bm25"
    manifest["features"] = [
        "field_index_cache",
        "query_emb_cache",
        "parallel_fields",
    ...
    logger.info("Migrated index to v2.1 at %s", index_dir)
...
class PrimeDiskIndexStore:
    """Load disk indexes and build MFARAll hybrid shortlists."""
    ...
    def shortlist_hybrid_dispatch(
        ...
        *,
        ...
        """Sync entry: sequential or asyncio parallel shortlist."""
        if not parallel:
        ...
        async def _run() -> tuple[list[str], np.ndarray, np.ndarray]:
            return await self.shortlist_hybrid_async(
                query_text,
                query_emb,
                k,
                timing=timing,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build STaRK-Prime disk index")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    build_faiss = os.getenv("ASMR_V21_MIGRATE_FAISS", "0") == "1"
