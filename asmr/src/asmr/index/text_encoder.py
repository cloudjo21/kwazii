...
class TextEncodingIndexer:
    ...
    def add_documents(self, doc_ids: List[str], texts: List[str], prompt_type: PromptType = PromptType.PASSAGE):
    ...
    def add_document(self, doc_id: str, text: str, prompt_type: PromptType = PromptType.PASSAGE):
        """Add a single document (wrapper for batch method)"""
        self.add_documents([doc_id], [text], prompt_type)
    
    def search(self, query: str, k: int = 10, prompt_type: PromptType = PromptType.QUERY) -> List[tuple]:
